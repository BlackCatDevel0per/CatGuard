...
class CreateArgument extends Argument
{
	...
	function execute( Player $sender, array $args ): bool
	{
		...
		{
			if( !$sender->hasPermission('catguard.all') )
			{
		...
		if( $main->calculateSize($pos1, $pos2) > $val['max_size'] and !$sender->hasPermission('catguard.all') )
		{
		...
		if( count($override) > 0 and !$sender->hasPermission('catguard.all') )
		{
