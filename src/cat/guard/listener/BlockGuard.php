...
use cat\guard\event\flag\FlagCheckByBlockEvent;
...
class BlockGuard implements Listener
{
	...
	function onSign( SignChangeEvent $event )
	{
		...
		if( strtolower($player->getName()) != $region->getOwner() and !$player->hasPermission('catguard.all') )
		{
			$api->sendWarning($player, $api->getValue('player_not_owner'));
			return;
		}

		$sign = $api->sign->get($rname, 'ошибка!');
	...
	private function isFlagDenied( Block $block, string $flag, Player $player = NULL ): bool
	{
		...
		if( !isset($region) )
		{
			if( $api->getValue('safe_mode', 'config') === TRUE )
			{
				if( isset($player) )
				{
					if( $player->hasPermission('catguard.all') )
					{
						return FALSE;
					}
